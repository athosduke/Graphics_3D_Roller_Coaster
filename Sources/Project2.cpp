/*
Project 2 Submission for CMPSC458
Name: Songmeng Wang
psu id: suw244
*/
...
std:printf(preamble.c_str());
	...
	unsigned int track_texture = loadTexture("../Project_2/Media/textures/metal.jpg");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindVertexArray(0);
		if (drawBoxes) {
			track.Draw(lightingShader_basic, track_texture);
		}
		else {
			track.Draw(reflectionShader, track_texture);
		}
		// Draw the heightmap
		if (drawHeightmap)
		{
		...
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // Rotated it towards the light.  Praise the sun
		...
		if (drawNormals)
		{
			...
			normalShader.setMat4("projection", projection);
			normalShader.setMat4("view", view);
			track.Draw(normalShader, track_texture);
		...
		if (followTrack) {
			camera.ProcessTrackMovement(deltaTime, track);
		}
	...
	if (somethingPressed && last_pressed < currentFrame - 0.5f || last_pressed == 0.0f)
	{
		...
		if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS) {
			if (followTrack) {
				...
				camera.Position = glm::vec3(2.0f, 0.0f, -12.0f);
